...
const seedMockDB = async () => {
  console.log('Seeding mock JSON database...');
  
  mockDbStore.deleteMany('roles');
  mockDbStore.deleteMany('users');
  mockDbStore.deleteMany('projects');
  mockDbStore.deleteMany('services');
  mockDbStore.deleteMany('teamMembers');
  mockDbStore.deleteMany('messages');

  const roles = mockDbStore.insertMany('roles', rolesData);
  const ownerRole = roles.find(r => r.name === 'Owner');
  const adminRole = roles.find(r => r.name === 'Admin');
  const engineerRole = roles.find(r => r.name === 'Software Engineer');
...
const seedDB = async () => {
  try {
    ...
    console.log('Connected to MongoDB. Seeding standard collections...');
    ...
    console.log('Switching to local mock JSON seeding...');
    await seedMockDB();
    process.exit(0);
  }
};
